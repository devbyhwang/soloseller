...
export function createApp(databasePath = process.env.DATABASE_PATH) {
...
export function startServer({ port = Number(process.env.PORT || 3000), host = '127.0.0.1', databasePath } = {}) {
  ...
  return new Promise((resolve, reject) => {
    server.once('listening', () => {
      ...
      console.log(`복숭아 주문대장이 http://${host}:${address.port} 에서 실행 중입니다.`)
      resolve(server)
    })
